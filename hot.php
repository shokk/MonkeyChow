...
$result = fof_get_items_pastweek($feed, $what, $when, $which, $howmany, $order, $tags, $search);
#print_r($result);
...
foreach($result as $row)
{
    ...
    $flag_sql = "SELECT `flag_id` FROM `" . $FOF_USERITEM_TABLE . "` WHERE `item_id`=\"" . $row['item_id'] . "\" AND `user_id`=" . current_user(); # AND `flag_id`!=1
	echo "SQL:: $flag_sql <br />\n";
